...
def prepare_data():
    ...
    with open("sample_movies.csv", "rb") as csvfile:
        ...
        for row in reader:
            ...
            match_year = re.search(".+(\d\d\d\d)", title)
            if match_year:
                year = match_year.group(1)
                data["year"] = int(year)
                # Now magic starts
                data["producer"] = random.sample(producers, 1).pop()
                data["director"] = random.sample(directors, 1).pop()
                data["duration"] = random.randrange(72, 180, 1)
